...
int MonClient::authenticate(double timeout)
{
  ...
  if (timeout > 0.0) {
    ...
    if (!authed) {
      ldout(cct, 0) << "authenticate timed out after " << timeout << "s"
		    << "with state = " << state << " and authenticate_err = "
		    << authenticate_err << dendl;
      assert(ceph::real_clock::now() - start >= dur);
...
int MonClient::_check_auth_rotating()
{
  ...
  if (!rotating_secrets->need_new_secrets(cutoff)) {
    ldout(cct, 0) << "_check_auth_rotating have uptodate secrets "
      "(they expire after " << cutoff << ")" << dendl;
    rotating_secrets->dump_rotating();
    return 0;
  }

  ldout(cct, 0) << "_check_auth_rotating renewing rotating keys (they "
...
void MonClient::_send_command(MonCommand& r)
{
  ...
  if (r.target_name.length() &&
    ...
    if (!monmap.contains(r.target_name)) {
      ldout(cct, 0) << " target " << r.target_name
...
void MonClient::handle_mon_command_ack(MMonCommandAck *ack)
{
  ...
  if (mon_commands.empty()) {
    ldout(cct, 0) << "handle_mon_command_ack " << ack->get_tid()
		   << " not found" << dendl;
    ack->put();
    return;
  }
  ...
  if (tid == 0)
    ldout(cct, 0) << "handle_mon_command_ack has tid 0, assuming it is "
		   << p->tid << dendl;
  ...
  ldout(cct, 0) << "handle_mon_command_ack " << r->tid << " " << r->cmd
		 << dendl;
...
void MonClient::handle_get_version_reply(MMonGetVersionReply* m)
{
  ...
  if (iter == version_requests.end()) {
    ...
    ldout(cct, 0) << __func__ << " finishing " << iter->first << " version "
		   << m->version << dendl;
    finisher.enqueue(cxx_function::in_place_t<Version_CB_Helper>{},
